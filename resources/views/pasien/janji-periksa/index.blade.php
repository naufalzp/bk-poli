                ...
                <select class="form-control" id="dokterSelect" name="id_jadwal_periksa" required>
                  ...
                  @foreach ($dokters as $dokter)
                    @foreach ($dokter->jadwalPeriksas as $jadwalPeriksa)
                      <option value="{{ $jadwalPeriksa->id }}">{{ $dokter->nama }} - Spesialis {{ $dokter->poli->nama }}|
                        {{ $jadwalPeriksa->hari }}, {{ $jadwalPeriksa->jam_mulai }} - {{ $jadwalPeriksa->jam_selesai }}</option>
                    @endforeach
                  @endforeach
                </select>
                <x-input-error :messages="$errors->get('id_jadwal_periksa')" />
              </div>
              <div class="form-group">
